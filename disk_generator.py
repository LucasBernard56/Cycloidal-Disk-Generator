...
with left_col:
    ...
    with st.form("input_form"):
        R_p_str = st.text_input(
            "Pin Circle Radius (Rₚ, mm)",
            value="50.0",
            help="Pitch circle radius of fixed pins (mm). Fractions allowed (e.g., 100/2)."
        )
        e_str = st.text_input(
            "Eccentricity (e, mm)",
            value="2.5",
            help="Cycloid eccentricity (mm). Fractions allowed."
        )
        r_str = st.text_input(
            "Pin Radius (r, mm)",
            value="2.0",
            help="Fixed pin radius (mm). Fractions allowed."
        )
        N = st.number_input(
            "Number of Pins (N)",
    ...
    # Convert text inputs
    R_p = parse_fraction(R_p_str)
    e   = parse_fraction(e_str)
    r   = parse_fraction(r_str)
    ...
    if x_expr and y_expr:
        ...
        .eq-card {{ border: 1px solid rgba(209, 213, 219, 1); border-radius:10px; background:#fff; overflow:hidden; }}
        .eq-header {{ display:flex; align-items:center; justify-content:space-between; padding:10px 12px; background:#f8fafc; border-bottom:1px solid #e6eef6; font-weight:700; font-family: ui-monospace; color:#0f172a; }}
        .eq-body {{ padding:12px; font-family: ui-monospace; white-space: pre-wrap; word-break: break-word; max-height:180px; overflow:auto; font-size:13px; line-height:1.35; }}
        .copy-btn {{ width:36px; height:36px; border-radius:8px; border:1px solid #cbd5e1; background:#fff; display:inline-flex; align-items:center; justify-content:center; cursor:pointer; transition: transform 0.06s ease, background 0.12s ease; }}
          ...
          <div class="eq-card">
            <div class="eq-header">
              ...
              <button id="copy_x" class="copy-btn" title="Copy x(t)">
                <svg viewBox="0 0 24 24"><path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 18H8V7h11v16z"/></svg>
              </button>
            </div>
            <pre class="eq-body" id="expr_x">{x_html}</pre>
          </div>
          <div class="eq-card">
            <div class="eq-header">
              <div>y(t)</div>
            ...
            <pre class="eq-body" id="expr_y">{y_html}</pre>
          </div>
        </div>
        ...
        document.getElementById('copy_x').addEventListener('click', ()=>_copyTextFrom('expr_x','copy_x'));
        document.getElementById('copy_y').addEventListener('click', ()=>_copyTextFrom('expr_y','copy_y'));
        </script>
        """
...
with right_col:
    ...
    if ok:
        ...
        if diag.get("has_singularity"):
            st.warning("Singularity detected in `atn` argument. Use t2 = 2*pi - 1e-6 in SolidWorks.")
    else:
